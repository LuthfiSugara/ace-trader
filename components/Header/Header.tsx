      ...
      <nav className='w-full max-w-[1400px] flex justify-between items-center gap-4 mx-auto'>
        <div className="w-full flex flex-row justify-between gap-4 items-center px-[40px] py-5" style={{display: layoutWidth < 950 ? 'none' : 'flex'}}>
          ...
          <div className='flex items-center gap-8 lg:pr-8'>
            ...
            <Link href={'https://ace-traderdashboard.propaccount.com/en/sign-in'} className='text-white text-sm md:text-lg font-medium border border-white py-1 px-6 rounded-full'>Login</Link>
          </div>
        </div>
          ...
          <div className={`${styles.menu} ${isOpen ? styles.open : ''}`}>
            <ul className={styles.menuItems}>
              <div className="flex flex-col gap-4">
                ...
                <Link onClick={onClose} href={'https://ace-traderdashboard.propaccount.com/en/sign-in'} className='text-white text-lg md:text-lg font-medium border-b-[1px] border-[#ffffff33] px-4 py-2'>Login</Link>
              </div>
            </ul>
          </div>
        </div>
